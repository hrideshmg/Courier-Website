      ...
      <div className="card h-100 px-lg-5 card-span" style={{ backgroundColor: '#3282B8' }}>
        <div className="card-body d-flex flex-column justify-content-around">
          <div className="text-center pt-5">
            ...
            <h5 className="my-4" style={{ color: 'white' }}>{title}</h5>
          </div>
          <p style={{ color: 'white', fontSize: '1.2rem' }}>{description}</p>
          <ul className="list" style={{ fontSize: '1.3rem', color: 'white' }}>
            {listItems.map((item, index) => (
              <li key={index} className="mb-2">
                <span className="me-2"><i className="fas fa-circle text-primary" style={{ fontSize: '.5rem' }}></i></span>
                {item}
              </li>
            ))}
          </ul>
          <div className="text-center my-5">
            <div className="d-grid">
              <button className="btn btn-outline-danger" type="submit" style={{ backgroundColor: '#1B262C', borderColor: '#1B262C', color: 'white' }}>Learn more</button>
            </div>
          </div>
        </div>
      </div>
